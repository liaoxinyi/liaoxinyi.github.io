...
- **可见性算法是怎么判断的？**
    ...
    - 判断`DB_TRX_ID`是否在`list`之中，如果在，则代表这个Read View生成时刻，DB_TRX_ID这个事务还在活跃，还没有Commit，它修改的数据，当前事务也是看不见的；如果不在，则说明，`DB_TRX_ID`这个事务在Read View生成之前就已经Commit了，它修改的结果，当前事务是能看见的
